...
function renderModeToggle() {
    ...
    [["auto", "🤖 Auto"], ["instant", "⚡ Instant"], ["deep", "🧠 Deep"]].forEach(([m, label]) => {
        ...
        b.title = m === "auto" ? "Deep tools → Ox Alpha, quick tools → Gemini/Groq"
                : m === "instant" ? "Everything → Gemini/Groq (fast)"
...
async function openAddToBasket(ticker) {
    ...
    try {
        ...
        $("#mbody").innerHTML = `
        <div class="kv" style="margin-bottom:14px"><b>Ticker</b><span style="font-weight:700;color:var(--navy)">${ticker}</span></div>
        <div class="kv">
            <b>Basket</b><span><select id="addBasket" style="width:280px">${options}<option value="__new__">➕ Create new basket…</option></select></span>
            <b>New basket name</b><span><input id="newBasketName" placeholder="e.g. 🆕 MY THESIS" style="width:280px;display:none"></span>
            <b>Weight</b><span><select id="addWeight" style="width:120px">
                <option value="0.5">0.5</option><option value="1.0" selected>1.0</option>
                <option value="1.5">1.5</option><option value="2.0">2.0</option>
                <option value="2.5">2.5</option><option value="3.0">3.0</option></select></span>
        </div>
        <div id="addResult" style="margin-top:12px"></div>
        <button class="btn gold" id="confirmAdd" style="margin-top:12px">✅ Add to SAF</button>`;
        $("#addBasket").onchange = () => { $("#newBasketName").style.display = $("#addBasket").value === "__new__" ? "inline-block" : "none"; };
        $("#confirmAdd").onclick = async () => {
            let basket = $("#addBasket").value;
            if (basket === "__new__") { basket = $("#newBasketName").value.trim(); if (!basket) { $("#addResult").innerHTML = `<span style="color:var(--neg)">Enter a basket name.</span>`; return; } }
            ...
            } catch (e) { $("#addResult").innerHTML = `<span style="color:var(--neg)">Error: ${e.message}</span>`; }
        };
...
async function openBasket(name) {
    ...
    try {
        ...
        for (const h of d.holdings) {
            rows += `<tr class="clickable" data-ticker="${h.ticker}"><td><b>${h.ticker}</b></td>
                <td class="r">${h.weight.toFixed(1)}</td>
                <td class="r">${h.price != null ? "$" + Number(h.price).toFixed(2) : "—"}</td>
                <td class="r">${fmtPct(h.ytd_pct)}</td></tr>`;
        }
        $("#mbody").innerHTML = `<div class="tbl"><table><tr><th>Ticker</th><th class="r">Weight</th><th class="r">Price</th><th class="r">YTD</th></tr>${rows}</table></div>`;
    } catch (e) { $("#mbody").innerHTML = `<div class="panel sell">Error: ${e.message}</div>`; }
}
    ...
    try {
        ...
        const memoHtml = m.business_summary
            ? `<div class="panel"><h3>📋 Company Memo — ${m.name || t}</h3>
                ...
                ${m.bottleneck ? `<div style="margin-top:8px">${m.bottleneck.is_bottleneck
                    ...
                    : `<span class="sig none">Bottleneck score ${m.bottleneck.rubric_total}/30 (below 22)</span>`}</div>` : ""}</div>`
        ...
        <div class="grid2" style="gap:12px">
            <div class="panel"><h3>Quality</h3><div class="kv">
                <b>Bars</b><span>${q.bars || 0}</span><b>Usable</b><span>${q.usable ? "✅" : "❌"}</span>
        ...
        document.querySelectorAll("#mbody .addBtn").forEach(b => b.onclick = (e) => { e.stopPropagation(); openAddToBasket(b.dataset.add); });

        api("/api/news?q=" + encodeURIComponent(t) + "&limit=3").then(hs => {
            const el = $("#mnews"); const items = (Array.isArray(hs.items) ? hs.items : []).slice(0, 3);
        ...
        api("/api/ticker/" + encodeURIComponent(t) + "/polymarket?limit=3").then(pm => {
            const el = $("#mpm"); const items = (Array.isArray(pm.items) ? pm.items : []).slice(0, 3);
            if (el) el.innerHTML = items.length
                ? items.map(mk => `<div style="margin:6px 0">${mk.link ? `<a class="nl" href="${mk.link}" target="_blank" rel="noopener">${mk.question} ↗</a>` : `<b>${mk.question}</b>`}
        ...
        $("#mrun").onclick = async () => {
            ...
            try {
                const res = await api("/api/pipeline/" + encodeURIComponent(t), { method: "POST" }, 900000);
                clearInterval(timer);
...
async function renderNews() {
    ...
    const load = async () => {
        ...
        try {
            const hs = await api("/api/news?q=" + encodeURIComponent(q) + "&limit=12");
            ...
            $("#nwire").innerHTML = items.length
                ? items.map(h => `<div class="panel">${h.link ? `<a class="nl" href="${h.link}" target="_blank" rel="noopener">${h.title} ↗</a>` : `<b>${h.title}</b>`}
                    <div class="dim">${h.source} · relevance ${h.relevance} ${(Array.isArray(h.keywords)?h.keywords:[]).map(k=>`<span class="chip">${k}</span>`).join("")}
                    ${h.is_signal ? `<span class="sig buy">🔥 SIGNAL</span>` : ""}</div></div>`).join("")
...
function renderScreener() {
    $("#main").innerHTML = `<div class="wrap"><div class="card"><h1>🔍 SAF Screener + AI</h1>
        ...
        <div class="grid-tools"><div class="card tools">
            ...
            <button data-t="saved">📂 Saved Outcomes</button>
...
const SCREENER = {
    async screen() {
        ...
        try {
            ...
            document.querySelectorAll(".addBtn").forEach(b => b.onclick = (e) => { e.stopPropagation(); openAddToBasket(b.dataset.add); });
            ...
            try {
                ...
                $("#dr").innerHTML = `<h3>Investability</h3><div class="panel"><div class="kv">
                    ...
                    <div class="row" style="margin-top:8px">
                        ...
                        <button class="btn ghost addBtn" data-add="${t}" style="margin-left:8px">➕ Add to SAF</button></div>`;
                document.querySelectorAll(".addBtn").forEach(b => b.onclick = (e) => { e.stopPropagation(); openAddToBasket(b.dataset.add); });
    ...
    async sup() {
        ...
        out.innerHTML = `<h2>🤖 AI Supply Chain Discovery</h2>
            <div class="sub">Uses ${MODE==="instant"?"⚡ Gemini/Groq (fast)":"🧠 Ox Alpha (deep)"} — switch mode above</div>
        ...
        $("#go").onclick = async () => {
            ...
            try {
                const d = await api("/api/screener/supply-chain", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({trend}) }, 900000);
                clearInterval(timer);
                const res = d.result || {};
                ...
                (Array.isArray(res.bottlenecks)?res.bottlenecks:[]).forEach((b, i) => {
                    html += `<div class="panel"><h3>Bottleneck #${i+1}: ${b.name || "?"}</h3>
                        ...
                        <div style="margin-top:6px">Tickers: ${(Array.isArray(b.tickers)?b.tickers:[]).map(t=>`<span class="chip" data-ticker="${t}">${t}</span>`).join("")}</div></div>`;
                });
                html += `<div class="row" style="margin-top:12px">
                    <button class="btn gold" id="saveSC">💾 Save Outcome</button>
                    <span id="saveSCStatus" class="sub"></span></div>`;
                $("#r").innerHTML = html;
                document.querySelectorAll("#r .chip[data-ticker]").forEach(b => b.onclick = (e) => { e.stopPropagation(); openTicker(b.dataset.ticker); });
                $("#saveSC").onclick = async () => {
                    $("#saveSCStatus").textContent = "Saving…";
                    try {
                        const sv = await api("/api/save/supply-chain", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({result: res}) });
                        $("#saveSCStatus").innerHTML = `<span style="color:var(--pos)">✅ Saved: ${sv.filename}</span>`;
                    } catch (e) { $("#saveSCStatus").innerHTML = `<span style="color:var(--neg)">Error: ${e.message}</span>`; }
                };
    ...
    async adeep() {
        ...
        $("#go").onclick = async () => {
            ...
            try {
                ...
                const col = d.memo && d.memo.includes("BUY") ? "buy" : d.memo && d.memo.includes("SKIP") ? "sell" : "hold";
                $("#r").innerHTML = `<div class="panel ${col}"><h3>FUND MANAGER MEMO — ${t}</h3><pre class="memo">${d.memo}</pre></div>
    ...
    async pm() {
        ...
        const renderMarkets = items => {
            ...
            $("#pmr").innerHTML = items.map(m => `<div class="panel">
                ...
                <div class="sub">YES ${m.yes != null ? Math.round(m.yes*100) + "%" : "—"} · 24h vol $${Math.round((m.volume24hr||0)/1000)}K ${m.endDate ? " · ends " + m.endDate : ""}</div></div>`).join("");
        };
    ...
    async saved() {
        const out = $("#out");
        out.innerHTML = `<h2>📂 Saved Outcomes</h2>
            <div class="row"><button class="btn" id="svSC">Supply Chain</button><button class="btn" id="svPL">Pipeline</button></div>
            <div id="svList" style="margin-top:12px"></div><div id="svDetail" style="margin-top:12px"></div>`;
        const loadList = async (kind) => {
            $("#svDetail").innerHTML = "";
            $("#svList").innerHTML = `<div class="panel"><span class="loader"></span> loading…</div>`;
            try {
                const d = await api("/api/saved/" + kind);
                if (!d.items.length) { $("#svList").innerHTML = `<div class="panel">No saved ${kind} outcomes yet.</div>`; return; }
                let html = `<div class="tbl"><table><tr><th>Name</th><th>Saved At</th><th>Size</th><th></th><th></th></tr>`;
                d.items.forEach(it => {
                    html += `<tr><td><b>${it.name}</b></td><td>${it.saved_at ? new Date(it.saved_at).toLocaleString() : "—"}</td>
                        <td>${it.size_kb} KB</td>
                        <td><button class="btn svLoad" data-f="${it.filename}" data-k="${kind}">Open</button></td>
                        <td><button class="btn ghost svDel" data-f="${it.filename}" data-k="${kind}">🗑️</button></td></tr>`;
                });
                $("#svList").innerHTML = html + "</table></div>";
                document.querySelectorAll(".svLoad").forEach(b => b.onclick = async () => {
                    const det = await api("/api/saved/" + b.dataset.k + "/" + encodeURIComponent(b.dataset.f));
                    $("#svDetail").innerHTML = `<div class="panel"><h3>${det.payload.name}</h3><pre class="memo">${JSON.stringify(det.payload.data, null, 2)}</pre></div>`;
                });
                document.querySelectorAll(".svDel").forEach(b => b.onclick = async () => {
                    await api("/api/saved/" + b.dataset.k + "/" + encodeURIComponent(b.dataset.f), { method: "DELETE" });
                    loadList(kind);
                });
            } catch (e) { $("#svList").innerHTML = `<div class="panel sell">Error: ${e.message}</div>`; }
        };
        $("#svSC").onclick = () => loadList("supply_chain");
        $("#svPL").onclick = () => loadList("pipeline");
        loadList("supply_chain");
    }
};
...
async function renderAgents() {
    $("#main").innerHTML = `<div class="wrap"><div class="card"><h1>⚛️ TradingAgents — 5-Stage Firm Simulation</h1>
        <div class="sub">mode: ${MODE} — ${MODE==="instant"?"⚡ Gemini/Groq fast":"🧠 Ox Alpha deep"}</div>
    ...
    $("#go").onclick = async () => {
        ...
        try {
            ...
            const a = res.state.trader.action;
            const col = a === "BUY" ? "buy" : a === "SELL" ? "sell" : "hold";
            $("#tl").insertAdjacentHTML("beforeend", `<div class="panel ${col}"><h3>FINAL SIGNAL: ${a}</h3><div class="sub">${res.state.trader.rationale || ""}</div></div>`);
            $("#tl").insertAdjacentHTML("beforeend", `<div class="row" style="margin-top:12px">
                <button class="btn gold" id="savePL">💾 Save Outcome</button>
                <span id="savePLStatus" class="sub"></span></div>`);
            $("#savePL").onclick = async () => {
                $("#savePLStatus").textContent = "Saving…";
                try {
                    const sv = await api("/api/save/pipeline", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({state: res.state}) });
                    $("#savePLStatus").innerHTML = `<span style="color:var(--pos)">✅ Saved: ${sv.filename}</span>`;
                } catch (e) { $("#savePLStatus").innerHTML = `<span style="color:var(--neg)">Error: ${e.message}</span>`; }
            };
...
async function renderPositions() {
    ...
    try {
        ...
        let html = `<div class="tbl"><table><tr><th>Ticker</th><th class="c">Dir</th><th class="r">Shares</th><th class="r">Entry</th>
            <th class="r">Last</th><th class="r">P/L</th><th class="r">Days</th><th class="r">Stop</th></tr>`;
        rows.forEach(p => {
            html += `<tr class="clickable" data-ticker="${p.ticker}"><td><b>${p.ticker}</b></td><td class="c">${p.direction || "LONG"}</td>
                <td class="r">${p.shares ?? "—"}</td><td class="r">$${Number(p.entry_price ?? p.entry ?? 0).toFixed(2)}</td>
